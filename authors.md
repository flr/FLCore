# Authors and Citation

## Authors

- **Iago Mosqueira**. Maintainer.
  [](https://orcid.org/0000-0002-3252-0591)

- **Laurence T. Kell**. Author.

- **Philippe Grosjean**. Author.

## Citation

Source:
[`inst/CITATION`](https://github.com/flr/FLCore/blob/master/inst/CITATION)

Kell, L.T., Mosqueira, I., Grosjean, P., Fromentin, J-M., Garcia, D.,
Hillary, R., Jardim, E., Mardle, S., Pastoors, M.A., Poos, J.J., Scott,
F., Scott, R.D. 2007. FLR: an open-source framework for the evaluation
and development of management strategies. ICES Journal of Marine
Science, 64: 640-646.

    @Article{,
      title = {FLR: an open-source framework for the evaluation and development of management strategies},
      author = {L.T. Kell and I. Mosqueira and P. Grosjean and J-M. Fromentin and D. Garcia and R. Hillary and E. Jardim and S. Mardle and M.A. Pastoors and J.J. Poos and F. Scott and R.D. Scott},
      journal = {ICES Journal of Marine Science},
      year = {2007},
      volume = {64},
      pages = {640-646},
    }
