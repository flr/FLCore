...
#' 
#' This method converts an FLQuant or any other FLR object composed of FLQuants
#' into a \code{\link[base]{data.frame}}.
#' 
#' For a single \link{FLQuant}, the \code{data.frame} returned has 7 columns:
#' \code{quant}, \code{year}, \code{unit}, \code{season}, \code{area},
...
#' dimensions. The \code{year} and \code{data} columns are of class
#' \link[base]{numeric}, while the other five are of class \link[base]{factor}.
#' 
#' When converting an \code{\linkS4class{FLCohort}} object, the \code{year}
#' column is substituted by \code{cohort}.
#' 
#' The \code{data.frame} returned for complex objects, i.e. those the inherit
#' from class \code{\link{FLComp}}, has an extra column, \code{slot}, that
#' holds the name of the slot in the original object.
#' 
#' The data.frame obtained from an \code{\linkS4class{FLQuants}} object also
...
#' 
#' Objects of class \link{FLQuants} can also be converted into a wide-format
#' table, where data from the list elements are placed in separate colums,
#' using \link{model.frame,FLlst-method}.
#' 
#' 
#' @name as.data.frame
