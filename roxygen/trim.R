...
#' 
#' Subsetting of FLR objects can be carried out using the dimension names by
#' using \code{trim}. A number of dimension names and selected dimensions are
...
#' one or more dimensions. If trim is applied on an FLQuant object of length 1
#' in its first dimension and with dimension name equal to 'all', values to
#' trim on specified for that dimension will be ignored. For example,
#' \code{\link{FLStock}} objects contain slots with length=1 on their first
#' dimension. Specifying values to trim along over the first dimension will
#' have no effect on those slots (\code{catch}, \code{landings},
#' \code{discards}, and \code{stock}). Calculations might need to be carried
#' out to recalculate those slots if their quant-structured counterparts are
