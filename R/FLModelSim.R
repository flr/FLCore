...
# mvrnorm {{{
setMethod("mvrnorm", signature(n="numeric", mu="FLSimpleModel", Sigma="missing",
	...
	function(n, mu) {
		...
		res <- FLPar(res)
		units(res) <- units(mu)
		return(FLSimpleModel(model=model(object), params=res))
	}
)

setMethod("mvrnorm", signature(n="FLSimpleModel", mu="missing", Sigma="missing",
	tol="missing", empirical="missing", EISPACK="missing"),
	function(n) {
...
# predict {{{
setMethod("predict", signature(object="FLSimpleModel"),
	function(object, ...) {
