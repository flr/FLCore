...
- model: Formula for the model, using the slot names *rec* and *ssb* to
  refer to the usual inputs

- logl: Function to calculate the loglikelihood of the given model when
  estimated through MLE (See [`fmle`](fmle-methods.md))

- initial: Function to provide initial values for all parameters to the
  minimization algorithms called by [`fmle`](fmle-methods.md) or
  [`nls`](https://rdrr.io/r/stats/nls.html). If required, this function
  also has two attributes, [`lower`](upperlower-methods.md) and
  [`upper`](upperlower-methods.md), that give lower and upper limits for
  the parameter values, respectively. This is used by some of the
  methods defined in [`optim`](https://rdrr.io/r/stats/optim.html), like
  `"L-BFGS-B"`.

The *model\<-* method for [`FLModel`](FLModel.md) can then be called
...
- ricker: Ricker stock-recruitment model fit: \$\$R = a S e^{-b S}\$\$
  *a* is related to productivity (recruits per stock unit at small stock
...
- shepherd: Shepherd stock-recruitment model fit: \$\$R = \frac{a
  ...
  (similar to *a* in the Ricker stock-recruit model), *b* the stock size
  above which density-dependent processes predominate over
  density-independent ones (also referred to as the threshold stock
  size), and *c* the degree of compensation.

- cushing: Cushing stock-recruitment model fit: \$\$R = a S e^{b}\$\$
  This model has been used less often, and is limited by the fact that
  it is unbounded for *b*\>=1 as *S* increases. (*a, b* \> 0).
...
- rickerSV: Fits a ricker stock-recruitment model parameterized for
  ...
  vbiomass}{spr0}}\$\$ \$\$b = \frac{\log(5 \cdot steepness)}{0.8 \cdot
  vbiomass}\$\$

- bevholtSV: Fits a Beverton-Holt stock-recruitment model parameterised
  for steepness and virgin biomass. \$\$a = \frac{4 \cdot vbiomass \cdot
  steepness}{(spr0 \cdot (5 \cdot steepness-1.0}\$\$ \$\$b =
  \frac{vbiomass (1.0-steepness)}{5 \cdot steepnes-1.0}\$\$
...
- sheperdSV: Fits a shepher stock-recruitment model parameterized for
  ...
  (\frac{0.2-steepness}{steepness (0.2)^c - 0.2})^ (\frac{-1.0}{c})\$\$
...
- bevholtAR1, rickerAR1, segregAR1: Beverton-Holt, Ricker and segmented
  regression stock-recruitment models with autoregressive normal log
  residuals of first order. In the model fit, the corresponding
  stock-recruit model is combined with an autoregressive normal log
  likelihood of first order for the residuals. If \\R_t\\ is the
  observed recruitment and \\\hat{R}\_t\\ is the predicted recruitment,
  an autoregressive model of first order is fitted to the log-residuals,
  \\x_t = \log(\frac{R_t}{\hat{R}\_t})\\. \$\$x_t=\rho x\_{t-1} + e\$\$
  where \\e\\ follows a normal distribution with mean 0: \\e \sim N(0,
  \sigma^2\_{AR})\\.
...
#> <bytecode: 0x55bb688e4970>
#> <environment: 0x55bb5a9d6c30>
#> 
#> $model
#> rec ~ a * ssb/(b + ssb)
#> <environment: 0x55bb5a9d6c30>
